...
const validBST = (tree, min = -Infinity, max = Infinity) => {
  // if no tree of reached a leaf return true of continue
  if (!tree) return true;
  // If the value if less than min or greater than max, return false
  if (tree.value < min || tree.value > max) return false;
  // Recursively call
  return (validBST(tree.left, min, tree.value) &&
